...
import { SECTION_COVER_PLACEHOLDER, type CollectionPageConfig, type CollectionSlug } from "@/app/lib/collectionPages";
...
export function ProductCollectionSections({
  ...
  return (
    <section className="luxe-section site-max site-px py-16 sm:py-20">
      ...
      <p className="mt-5 max-w-3xl text-sm leading-relaxed text-[var(--color-ink-muted)] sm:text-base">{description}</p>

      <ul className="mt-12 grid gap-5 sm:gap-6 sm:grid-cols-2 lg:grid-cols-4">
        {sections.map((s) => (
          <li key={s.id}>
            <CategoryCollectionCard item={subsectionToCardItem(s, collectionSlug)} ctaLabel="Open section →" />
          </li>
        ))}
      </ul>
    </section>
